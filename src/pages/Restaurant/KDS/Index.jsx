...
export default function RestaurantKDSPage() {
    ...
    try {
      ...
      if (posOrdersJson) {
        ...
        const mappedPosKots = posOrders.map((po) => ({
          ...
          elapsedSeconds: Math.floor((Date.now() - new Date(po.firedAt || Date.now()).getTime()) / 1000),
        ...
        // Deduplicate
        const existingKotNumbers = new Set(combined.map((k) => k.kotNumber));
  ...
  const filteredOrders = useMemo(() => {
    return kitchenOrders.filter((kot) => {
      ...
      if (selectedStatusFilter === 'ACTIVE') {
        matchesStatus = kot.status === 'QUEUED' || kot.status === 'PREPARING';
    ...
    setKitchenOrders((prev) => {
      return prev.map((kot) => {
        if (kot.id === kotId) {
          ...
          if (allServed) overallKotStatus = 'SERVED';
          else if (allReady) overallKotStatus = 'READY';
          else if (updatedItems.some((i) => i.status === 'PREPARING')) overallKotStatus = 'PREPARING';
  ...
  const getTimerClass = (seconds, priority) => {
    if (priority === 'RUSH' || seconds > 900) return styles.timerRed; // > 15 mins
    if (seconds > 300) return styles.timerAmber; // 5-15 mins
    return styles.timerGreen; // < 5 mins
  };
    ...
    <div className={styles.page} data-testid="restaurant-kds-page">
      ...
      <header className={styles.header}>
        <div className={styles.headerTitleGroup}>
          ...
          <p>Live Kitchen Orders (KOT) Preparation & Station Dispatch Monitor</p>
        </div>
        <div className={styles.headerActions}>
          <Badge variant="vip">Live KDS Feed Active</Badge>
          <Link to="/restaurant/pos" className={styles.posNavLink} data-testid="link-to-pos">
      ...
      <div className={styles.controlsCard}>
        <div className={styles.stationTabs}>
          {KDS_STATIONS.map((st) => (
            <button
              key={st.id}
              className={`${styles.stationBtn} ${selectedStation === st.code ? styles.stationBtnActive : ''}`}
        ...
        <div className={styles.statusFilterGroup}>
          ...
          {[
            ...
            <button
              key={f.id}
              className={`${styles.statusPill} ${selectedStatusFilter === f.id ? styles.statusPillActive : ''}`}
      ...
      <div className={styles.ticketGrid}>
        {filteredOrders.map((kot) => {
          ...
          if (kot.status === 'PREPARING') cardClass = styles.ticketCardPreparing;
          if (kot.status === 'READY') cardClass = styles.ticketCardReady;
          if (kot.status === 'SERVED') cardClass = styles.ticketCardServed;

          return (
            <div key={kot.id} className={`${styles.ticketCard} ${cardClass}`}>
              {/* Header */}
              <div className={styles.ticketHeader}>
                ...
                <div className={`${styles.timerBadge} ${getTimerClass(kot.elapsedSeconds, kot.priority)}`}>
              ...
              <div className={styles.itemList}>
                {kot.items.map((item) => (
                  <div key={item.id} className={styles.itemRow}>
                        ...
                        onClick={() => {
                          const nextStatus =
                            item.status === 'QUEUED'
                              ...
                              ? 'READY'
                              : 'SERVED';
                          handleUpdateItemStatus(kot.id, item.id, nextStatus);
                        }}
                        className={`${styles.itemStatusPill} ${
                          item.status === 'QUEUED'
                            ? styles.statusQueued
                            : item.status === 'PREPARING'
                    ...
                    {item.note && (
                      <div className={styles.itemNote}>Note: &quot;{item.note}&quot;</div>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className={styles.ticketFooter}>
                ...
                {kot.status === 'SERVED' && (
                  <div className={styles.completedTag}>✓ Order Completed</div>
                )}
              </div>
            </div>
          );
        ...
        {filteredOrders.length === 0 && (
          <div className={styles.emptyState}>
            ...
            <p>
              There are no active orders matching station &quot;{selectedStation}&quot; and status &quot;{selectedStatusFilter}&quot;.
            </p>
          </div>
        )}
      </div>
